...
def resolve_ingredient_cost(ingredient_id, recipe_unit, quantity=1):
    ...
    try:
        ...
        # NEW: use structured fields instead of parsing size string
        ...
        if quote_unit != "lb":
            cursor.execute("""
                ...
                WHERE ingredient_id = %s AND from_unit = %s AND to_unit = 'lb'
...
def resolve_item_cost(item_id, recipe_unit, quantity=1, visited=None):
    ...
    if item_id in visited:
        return {"status": "error", "issue": "circular_dependency", "item_id": item_id}
    ...
    if not item or not item["is_prep"]:
        return {"status": "error", "issue": "not_prep_item", "item_id": item_id}

    try:
        yield_qty = float(item["yield_qty"])
        yield_unit = item["yield_unit"].strip().lower()
    except:
        return {"status": "error", "issue": "missing_or_invalid_yield", "item_id": item_id}
    ...
    for c in components:
        if c["source_type"] == "ingredient":
            from cost_resolver import resolve_ingredient_cost
    ...
    if issues:
        return {"status": "error", "issue": "child_resolution_error", "details": issues}

    if yield_unit != recipe_unit:
        cursor.execute("""
            SELECT factor FROM ingredient_conversions
            WHERE ingredient_id IS NULL AND from_unit = %s AND to_unit = %s
        """, (yield_unit, recipe_unit))
        row = cursor.fetchone()
        if not row:
            return {"status": "error", "issue": "missing_conversion", "from": yield_unit, "to": recipe_unit}
